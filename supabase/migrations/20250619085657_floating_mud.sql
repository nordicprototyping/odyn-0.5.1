/*
  # Update Risk Categories and Add AI Risk Fields

  1. New Risk Categories
    - Adds new values to the risk_category enum type
    - Maps existing categories to new values
  
  2. New Fields
    - Adds fields for AI-generated risks
    - Adds source reference fields for risks
  
  3. Indexing
    - Creates appropriate indexes for new columns
*/
...
-- Commit the transaction to ensure enum values are available
COMMIT;
...
UPDATE risks SET category = 
  CASE 
    WHEN temp_category IN ('security', 'operational', 'financial', 'strategic') THEN 'physical_security_vulnerabilities'
    WHEN temp_category = 'environmental' THEN 'environmental_hazards'
    WHEN temp_category = 'technical' THEN 'infrastructure_failure'
    WHEN temp_category IN ('compliance', 'reputational') THEN 'personnel_safety_security'
    ELSE 'physical_security_vulnerabilities'
  END::risk_category;
